...
export class ColeccionCartas {
   ...
   * @param callback Función de devolución de llamada (callback) que recibe un error si lo hay y un mensaje de confirmación si la operación se realiza con éxito.
   */
  addCarta(user: string, carta: Carta, callback: (error: Error | undefined | string, message: string | undefined) => void): void {
    const userFolder = `./data/${user}`;
    fs.mkdir(userFolder, { recursive: true }, (err) => {
      if (err) {
        return callback(err, undefined);
      }
      const filePath = `./data/${user}/${carta._id}.json`;
      fs.access(filePath, fs.constants.F_OK, (err) => {
        if (!err) {
          return callback(`Card with id ${carta._id} already exists in ${user} collection!`, undefined);
        }
        fs.writeFile(filePath, JSON.stringify(carta), (err) => {
          if (err) {
            return callback(err, undefined);
          }
          callback(undefined, `New card ${carta._nombre} added to ${user} collection!`);
        });
      });
    });
  }
   ...
   * @param callback Función de devolución de llamada (callback) que recibe un error si lo hay y un mensaje de confirmación si la operación se realiza con éxito.
   */
  removeCarta(user: string, id: number, callback: (error: string | undefined | Error, message: string | undefined) => void): void {
    const userFolder = `./data/${user}`;
    fs.access(userFolder, fs.constants.F_OK, (error) => {
      if (error) {
        return callback(`User ${user} has never added a card and does not exist!`, undefined);
      }
      fs.access(`./data/${user}/${id}.json`, fs.constants.F_OK, (error) => {
        if (error) {
          return callback(`Card with id ${id} does not exist in ${user} collection!`, undefined);
        }
        fs.unlink(`./data/${user}/${id}.json`, (error) => {
          if (error) {
            return callback(error, undefined);
          }
          callback(undefined, `Card with id ${id} removed from ${user} collection!`);
        });
      });
    });
  }
